...
class ImageDataset(Dataset):
    def __init__(self, split='train', crop_size=(512, 512)):
        super().__init__()
        self.split = split
...
def train(epoch, model, train_loader, criterion, optimizer):
    ...
    epoch_records = None
    for sample_idx, (images, labels) in enumerate(train_loader):
...
def valid(epoch, model, valid_loader, criterion):
    ...
    epoch_records = None
    for sample_idx, (images, labels) in enumerate(valid_loader):
        with torch.no_grad():
            images = images.float().to(device)
            labels = labels.float().to(device)
            outputs = model(images)
...
def main():
    ...
    }, save_file='pretrained/unet.pth.tar', is_best=False)
